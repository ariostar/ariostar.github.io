...
$h2(k,i) = (h1(k) + c1*i + c2*i^2) \pmod M$
...
探测公式一般选择：$h(k, i) = (h1(k) + i * h2(k)) \pmod M$
...
即：探测序列 $(h1(k) + i * h2(k)) \pmod M$ 在 i 从 1 增加到 M-1 的过程中，**要证明：**
...
当 n=0 时，$n*M = 0 \Longrightarrow (i2-i1) = 0$，不可能，因为 $i2 \not= i1$；
当 n=1 时，$n*M = M \Longrightarrow (i2-i1) = M$，不可能，因为 i2 < M 且 i1 < M，(i2-i1) 不可能大于等于M。
